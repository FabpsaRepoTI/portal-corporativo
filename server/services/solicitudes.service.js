...
async function crearSolicitud({ idServicio, idPrioridad, titulo, descripcion, idUsuario, nombreUsuario, areaUsuario, sitioUsuario }) {
  const pool = await getPool();

  // 1. Obtener SLA de la prioridad
  const prioRes = await pool.request()
  ...
  const { slaRespuestaHrs, slaResolucionHrs, prioridad, colorHex } = prioRes.recordset[0];

  // 2. Calcular fechas límite
  const ahora = new Date();
  const fechaLimiteResp  = new Date(ahora.getTime() + slaRespuestaHrs  * 3600000);
  const fechaLimiteResol = new Date(ahora.getTime() + slaResolucionHrs * 3600000);
  ...
  const insertRes = await pool.request()
    ...
    .input("fechaLimiteResol", fechaLimiteResol)
    .query(`
...
module.exports = { crearSolicitud, obtenerSolicitudesUsuario };
